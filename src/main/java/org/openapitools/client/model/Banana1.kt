...
class Banana {
    ...
    fun lengthCm(lengthCm: BigDecimal?): Banana {
    ...
    override fun equals(o: Any?): Boolean {
        ...
        val banana = o as Banana
        return lengthCm == banana.lengthCm
    }
    ...
    class CustomTypeAdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T> {
            if (!Banana::class.java.isAssignableFrom(type.rawType)) {
                return null // this class only serializes 'Banana' and its subtypes
            }
            val elementAdapter = gson.getAdapter(JsonElement::class.java)
            val thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Banana::class.java))
            return object : TypeAdapter<Banana>() {
                @Throws(IOException::class)
                override fun write(out: JsonWriter, value: Banana) {
                    val obj = thisAdapter.toJsonTree(value).getAsJsonObject()
                    elementAdapter.write(out, obj)
                }

                @Throws(IOException::class)
                override fun read(`in`: JsonReader): Banana {
    ...
    companion object {
        ...
        fun validateJsonElement(jsonElement: JsonElement?) {
            if (jsonElement == null) {
                require(openapiRequiredFields!!.isEmpty()) {  // has required fields but JSON element is null
                    String.format("The required field(s) %s in Banana is not found in the empty JSON string", openapiRequiredFields.toString())
                }
            }
            val entries = jsonElement!!.getAsJsonObject().entrySet()
            // check to see if the JSON string contains additional fields
            for ((key) in entries) {
                require(openapiFields!!.contains(key)) { String.format("The field `%s` in the JSON string is not defined in the `Banana` properties. JSON: %s", key, jsonElement.toString()) }
            }
         ...
         */
        @Throws(IOException::class)
        fun fromJson(jsonString: String?): Banana {
            return JSON.getGson().fromJson(jsonString, Banana::class.java)
        }
    }
}
