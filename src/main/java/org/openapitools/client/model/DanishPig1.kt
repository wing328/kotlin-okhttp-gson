...
class DanishPig {
    ...
    fun className(className: String?): DanishPig {
    ...
    override fun equals(o: Any?): Boolean {
        ...
        val danishPig = o as DanishPig
        return className == danishPig.className
    }
    ...
    class CustomTypeAdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T> {
            if (!DanishPig::class.java.isAssignableFrom(type.rawType)) {
                return null // this class only serializes 'DanishPig' and its subtypes
            }
            val elementAdapter = gson.getAdapter(JsonElement::class.java)
            val thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(DanishPig::class.java))
            return object : TypeAdapter<DanishPig>() {
                @Throws(IOException::class)
                override fun write(out: JsonWriter, value: DanishPig) {
                    val obj = thisAdapter.toJsonTree(value).getAsJsonObject()
                    elementAdapter.write(out, obj)
                }

                @Throws(IOException::class)
                override fun read(`in`: JsonReader): DanishPig {
    ...
    companion object {
        ...
        fun validateJsonElement(jsonElement: JsonElement?) {
            if (jsonElement == null) {
                require(openapiRequiredFields!!.isEmpty()) {  // has required fields but JSON element is null
                    String.format("The required field(s) %s in DanishPig is not found in the empty JSON string", openapiRequiredFields.toString())
                }
            }
            val entries = jsonElement!!.getAsJsonObject().entrySet()
            // check to see if the JSON string contains additional fields
            for ((key) in entries) {
                require(openapiFields!!.contains(key)) { String.format("The field `%s` in the JSON string is not defined in the `DanishPig` properties. JSON: %s", key, jsonElement.toString()) }
            }
         ...
         */
        @Throws(IOException::class)
        fun fromJson(jsonString: String?): DanishPig {
            return JSON.getGson().fromJson(jsonString, DanishPig::class.java)
        }
    }
}
